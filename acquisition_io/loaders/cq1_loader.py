...
def get_tp_df(path: pl.Path, ome_xml_filename: str):  # noqa: C901, get bent flake8
    ...
    if (rows, cols) in PLATE_WELL_LUT:
        ...
        warnings.warn(
            "Could not find well names for this plate size. Falling back to"
            "integer-based well names. Consider adding plate size to"
            "PLATE_WELL_LUT in cq1_loader.py.",
            stacklevel=2,
        )
    ...
    for channel in ex_px.channels:
        if channel.illumination_type.value == "Epifluorescence":  # type: ignore
            ex_wl = channel.excitation_wavelength
            em_wl = channel.emission_wavelength
            if ex_wl is None or em_wl is None:
                raise ValueError(
                    "Excitation or emission wavelength is missing for channel"
                    f"{channel}."
                )
            ex, em = (
                int(ex_wl),
    ...
    if len(channels) != len(set(channels)):
        ...
        channels = [str(c) for c in range(len(channels))]
...
def get_experiment_df_detailed(
    ...
    """
    Indexes a CQ1 experiment directory by timepoint, channel, region, field, and
    z-slice.
    ...
    ordinal_time : bool
        If True, reindex the timepoints to be ordinal integers. This is useful for
        displaying longitudinal experiments in a more human-readable format. Only
        applies to multi-timepoint experiments.
    ...
    if None in tps:
        raise ValueError(
            f"One or more acquisition directories in {base_path} are not named"
            "according to the CQ1 convention, and cannot be parsed.Please verify"
            "acquisition directory names."
        )
    ...
    if not all(c == example_chans for c in channels.values()):
        raise ValueError(
            "Channels are not homogeneous across all acquisitions; this is not"
            f"supported.\n{tablefmt(channels)}"
        )
    ...
    if not all(r == example_regions for r in regions.values()):
        raise ValueError(
            "Regions are not homogeneous across all acquisitions; this is not"
            f"supported.\n{tablefmt(regions)}"
        )
    ...
    if not all(f == example_fields for f in fields.values()):
        raise ValueError(
            "Fields are not homogeneous across all acquisitions; this is not"
            f"supported.\n{tablefmt(fields)}"
        )
...
def load_df(df, shape, attrs) -> xr.DataArray:
    def read_img(path):
        ...
        if path is None:
            logger.warning(
                "MeasurementResult.ome.xml is missing an image. This is likely the"
                "result of an acquisition error! Replacing with NaNs..."
            )
            return np.full(shape, np.nan)
        elif not path.exists():
            logger.warning(
                f"Could not find image at {path}, even though its existence is"
                "recorded in MeasurementResult.ome.xml. The file may have been"
                "moved or deleted. Replacing with NaNs..."
            )
            return np.full(shape, np.nan)
...
def load_cq1(base_path: pl.Path) -> xr.DataArray:
    ...
    ----------
    base_path : pl.Path
        Path to the directory containing the experiment. This can either be a
        directory containing one or more acquisition subdirectories, or an
        individual acquisition directory.
